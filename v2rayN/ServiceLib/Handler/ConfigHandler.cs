...
public static class ConfigHandler
{
    ...
    public static Config? LoadConfig()
    {
        ...
        config.SniSpoofingItem.ListenPort = config.SniSpoofingItem.ListenPort is > 0 and <= 65535
            ? config.SniSpoofingItem.ListenPort
            : 40443;
        config.SniSpoofingItem.ConnectPort = config.SniSpoofingItem.ConnectPort is > 0 and <= 65535
            ? config.SniSpoofingItem.ConnectPort
            : 443;
        config.SniSpoofingItem.FakeSni ??= "auth.vercel.com";
        config.MhrItem ??= new();
        config.MhrItem.Variant = config.MhrItem.Variant is "mhr" or "mhr-cfw" ? config.MhrItem.Variant : "mhr";
        config.MhrItem.ScriptId ??= string.Empty;
        config.MhrItem.AuthKey ??= string.Empty;
