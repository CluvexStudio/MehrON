...
MehrON packages the source releases of [MasterHttpRelayVPN](https://github.com/masterking32/MasterHttpRelayVPN) and [mhr-cfw](https://github.com/denuitt1/mhr-cfw) under `bin/mhr/`.

When a user saves MHR Settings, MehrON copies the selected `config.example.json` to `config.json` and writes `script_id`, `auth_key`, `http_port` or `listen_port`, and `socks5_port`.
